...
import { ArrowRight, ArrowLeft, Heart, Zap, Plus } from 'lucide-react';
import { toast } from 'react-toastify';

const workStyles = [
  { id: 'style-creative', label: 'Creative', icon: '🎨' },
  { id: 'style-logical', label: 'Logical', icon: '🧠' },
  { id: 'style-hands', label: 'Hands-on', icon: '🛠️' },
  { id: 'style-team', label: 'Team Work', icon: '🤝' },
  { id: 'style-indie', label: 'Independent', icon: '👤' },
];
...
const StepTwo = ({ formData, updateFormData, onNext, onPrev }) => {
  ...
  const cardStyle = (isSelected) => ({
    background: isSelected ? 'rgba(124, 58, 237, 0.2)' : 'rgba(45, 55, 72, 0.3)',
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs mb-4 text-gray-400">Select your preferred environments or work styles.</p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
            ...
            {selectedPrefs.filter(p => !workStyles.find(ws => ws.label === p)).map(tag => (
               <motion.button
               key={tag}
               type="button"
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={addCustomTag} className="p-2 bg-gray-800 rounded-xl text-gray-400 hover:text-white">
              <Plus size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <div>
            <label className="block text-sm font-semibold mb-2 text-white">What motivates you?</label>
            <div className="relative">
              <Heart className="absolute left-3 top-3 text-gray-500" size={18} />
              <input 
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-white">Current Situation</label>
            <div className="relative">
        ...
        <div>
          <label className="block text-sm font-semibold mb-1 text-white">Anything else we should know?</label>
          <textarea
