...
const learningStyles = [
  { id: 'style-visual', value: 'visual', label: 'Visual', desc: 'Diagrams, videos, illustrations', icon: '👁️' },
  { id: 'style-reading', value: 'reading', label: 'Reading', desc: 'Articles, docs, books', icon: '📖' },
  { id: 'style-handson', value: 'hands-on', label: 'Hands-on', desc: 'Projects, coding exercises', icon: '🛠️' },
  { id: 'style-structured', value: 'structured', label: 'Structured', desc: 'Courses with clear progression', icon: '📋' },
];

const resourceTypes = [
  { id: 'res-docs', value: 'documentation', label: 'Official Docs', icon: '📄' },
  ...
  { id: 'res-interactive', value: 'interactive', label: 'Interactive (Codecademy, etc.)', icon: '💻' },
  { id: 'res-books', value: 'books', label: 'Books / PDFs', icon: '📚' },
];


const StepTwo = ({ formData, updateFormData, onNext, onPrev  }) => {4
     ...
     <form onSubmit={handleSubmit(onSubmit)}>
      ...
      >
            ...
            {learningStyles.map((style) => {
              ...
              return (
                <motion.button
                  ...
                  style={{
                    background: isSelected ? 'rgba(124, 58, 237, 0.15)' : 'rgba(45, 55, 72, 0.3)',
                    border: isSelected ? '1px solid rgba(124, 58, 237, 0.5)' : '1px solid rgba(45, 55, 72, 0.5)',
                    boxShadow: isSelected ? '0 0 12px rgba(124, 58, 237, 0.15)' : 'none',
                  }}
                >
                  <span className="text-2xl">{style.icon}</span>
                  <span className="text-sm font-semibold" style={{ color: isSelected ? '#F9FAFB' : '#9CA3AF' }}>
                    {style.label}
                  </span>
                  <span className="text-xs text-center" style={{ color: '#6B7280' }}>{style.desc}</span>
                  {isSelected && (
                    <div
                    ...
                    >
                      <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                        <path d="M2 5l2 2 4-4" stroke="#9F67FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </div>
                  )}
                </motion.button>
        ...
        <div>
          ...
          <p className="text-xs mb-4" style={{ color: '#9CA3AF' }}>
            SkillOrbit will prioritize these resource formats when curating your tasks.
          </p>
            ...
            {resourceTypes.map((res) => {
              ...
              return (
                <motion.button
                  ...
                  style={{
                    background: isSelected ? 'rgba(124, 58, 237, 0.12)' : 'rgba(45, 55, 72, 0.3)',
                    border: isSelected ? '1px solid rgba(124, 58, 237, 0.4)' : '1px solid rgba(45, 55, 72, 0.5)',
                  }}
                >
                  <span className="text-xl flex-shrink-0">{res.icon}</span>
                  <span className="text-sm font-medium" style={{ color: isSelected ? '#F9FAFB' : '#9CA3AF' }}>
                    {res.label}
                  </span>
                  {isSelected && (
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                ...
                style={{ background: 'rgba(45, 55, 72, 0.3)', border: '1px solid rgba(45, 55, 72, 0.5)' }}
              >
                <div>
                  <p className="text-sm font-medium" style={{ color: '#F9FAFB' }}>{toggle.label}</p>
                  <p className="text-xs mt-0.5" style={{ color: '#9CA3AF' }}>{toggle.desc}</p>
                </div>
                <button
                  type="button"
                  onClick={() => updateFormData({ [toggle.key]: !toggle.value })}
                  ...
                  style={{
                    background: toggle.value
                      ...
                      : 'rgba(45, 55, 72, 0.6)',
                    boxShadow: toggle.value ? '0 0 10px rgba(124, 58, 237, 0.4)' : 'none',
                  }}
